...
async def write_update(
    ...
    # Stored XSS 방지
    form_data.wr_content = ContentSanitizer().get_cleaned_data(form_data.wr_content)
    # form_data.wr_subject = bleach.clean(form_data.wr_subject, tags=allowed_title_tags, attributes=allowed_attrs, css_sanitizer=allowed_css_sanitizer)
    # form_data.wr_content = bleach.clean(form_data.wr_content, tags=allowed_tags, attributes=allowed_attrs, css_sanitizer=allowed_css_sanitizer)
...
async def write_comment_update(
    ...
    if form.w == "c":
        ...
        comment.wr_content = ContentSanitizer().get_cleaned_data(form.wr_content)
