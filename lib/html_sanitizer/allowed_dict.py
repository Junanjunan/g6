...
해당 태그에만 허용되는 속성이 아닌, 공통 허용 속성이므로 착오 없으시길 바랍니다.

lxml 라이브러리에서 allow_tags, allow_attrs에 대한 if문 분기처리 특성상
허용하려는 태그나, 속성이 없는 경우에 dict() 또는 set()으로 사용하면 blocklist 방식으로 동작하게 됩니다.
해당 코드는 allowlist 방식으로 동작하도록 작성 되었으므로,
allow_tags, safe_attrs가 set()과 같이 빈값으로 넘어가지 않아야 합니다.
그에 따라 모든 태그 또는 모든 속성을 허용하지 않으려는 경우에는 {'None': {''}}로 값을 설정 했습니다.
"""
# BaseSanitizer
